...
class DataTransaksiMCtoSS:
    ...
    def transfer_bom_master(self, model_name, fields, description, date_from, date_to):
        try:
            ...
            # STEP A - Mapping product.template dengan default_code
            ...
            # STEP B - Mapping product.product dengan default_code (header)
            ...
            # STEP C - Mapping bom.line (component items) dan product.uom
            ...
            # Dapatkan semua product_id dari bom lines
            bom_component_ids = []
            for line in bom_lines:
                if line.get('product_id'):
                    if isinstance(line['product_id'], list):
                        bom_component_ids.append(line['product_id'][0])
                    else:
                        bom_component_ids.append(line['product_id'])

            # STEP D - Mapping UOM dari source ke target (DIPINDAHKAN KE ATAS)
            # Kumpulkan semua UOM ID dari bom header dan lines
            uom_ids = []
            
            # UOM dari bom header
            for record in transaksi_bom_master:
                if record.get('product_uom_id'):
                    if isinstance(record['product_uom_id'], list):
                        uom_ids.append(record['product_uom_id'][0])
                    else:
                        uom_ids.append(record['product_uom_id'])
            
            # UOM dari bom lines
            for line in bom_lines:
                if line.get('product_uom_id'):
                    if isinstance(line['product_uom_id'], list):
                        uom_ids.append(line['product_uom_id'][0])
                    else:
                        uom_ids.append(line['product_uom_id'])

            uom_name_to_target_id = {}
            uom_source_id_to_name = {}
            
            if uom_ids:
                uom_source = self.source_client.call_odoo('object', 'execute_kw', self.source_client.db,
                    self.source_client.uid, self.source_client.password,
                    'uom.uom', 'search_read',
                    [[['id', 'in', list(set(uom_ids))]]],
                    {'fields': ['id', 'name', 'category_id']})

                uom_target = self.target_client.call_odoo('object', 'execute_kw', self.target_client.db,
                    self.target_client.uid, self.target_client.password,
                    'uom.uom', 'search_read',
                    [[['name', 'in', [uom['name'] for uom in uom_source]]]],
                    {'fields': ['id', 'name', 'category_id']})

                uom_name_to_target_id = {uom['name']: uom['id'] for uom in uom_target}
                uom_source_id_to_name = {uom['id']: uom['name'] for uom in uom_source}
                
                print(f"🔧 Mapping UOM: {uom_name_to_target_id}")

            # STEP E - Mapping product untuk bom lines (DIPERBAIKI)
            product_line_source = self.source_client.call_odoo('object', 'execute_kw', self.source_client.db,
                self.source_client.uid, self.source_client.password,
                'product.product', 'search_read',
                [[['id', 'in', list(set(bom_component_ids))]]],
                {'fields': ['id', 'default_code', 'name']})  # Tambah field name untuk debugging

            product_line_id_to_default_code = {p['id']: p['default_code'] for p in product_line_source if p.get('default_code')}
            product_line_id_to_name = {p['id']: p['name'] for p in product_line_source if p.get('name')}  # Untuk debugging
            
            print(f"🔧 Product line source: {[(p['id'], p['default_code'], p['name']) for p in product_line_source if p.get('default_code')]}")

            # Cari produk di target berdasarkan default_code DAN name (fallback)
            default_codes_to_search = list(set([p['default_code'] for p in product_line_source if p.get('default_code')]))
            product_names_to_search = list(set([p['name'] for p in product_line_source if p.get('name')]))
            
            product_line_target = self.target_client.call_odoo('object', 'execute_kw', self.target_client.db,
                self.target_client.uid, self.target_client.password,
                'product.product', 'search_read',
                [['|', ['default_code', 'in', default_codes_to_search], 
                         ['name', 'in', product_names_to_search]]],
                {'fields': ['id', 'default_code', 'name']})

            # Buat mapping dengan prioritas default_code, lalu name
            default_code_to_target_line_product_id = {}
            product_name_to_target_line_product_id = {}
            
            for p in product_line_target:
                if p.get('default_code'):
                    default_code_to_target_line_product_id[p['default_code']] = p['id']
                if p.get('name'):
                    product_name_to_target_line_product_id[p['name']] = p['id']
            
            print(f"🔧 Product line target: {[(p['id'], p['default_code'], p['name']) for p in product_line_target]}")

            # STEP F - Ambil semua BoM yang sudah ada di target (untuk pengecekan update)
            ...
            def process_bom(record):
                ...
                bom_line_items = [line for line in bom_lines if line.get('bom_id') and 
                                (line['bom_id'][0] if isinstance(line['bom_id'], list) else line['bom_id']) == record['id']]
                
                bom_line_vals = []
                missing_products = []

                for line in bom_line_items:
                    src_line_product_id = line['product_id'][0] if isinstance(line['product_id'], list) else line['product_id']
                    src_line_default_code = product_line_id_to_default_code.get(src_line_product_id)
                    src_line_name = product_line_id_to_name.get(src_line_product_id)
                    
                    # Cari target product ID dengan prioritas default_code, lalu name
                    target_line_product_id = None
                    
                    if src_line_default_code:
                        target_line_product_id = default_code_to_target_line_product_id.get(src_line_default_code)
                    
                    if not target_line_product_id and src_line_name:
                        target_line_product_id = product_name_to_target_line_product_id.get(src_line_name)
                    
                    if not target_line_product_id:
                        missing_products.append(f"{src_line_default_code or 'No Code'}: {src_line_name or 'No Name'}")
                        print(f"❌ Produk tidak ditemukan di target: ID {src_line_product_id}, Code: {src_line_default_code}, Name: {src_line_name}")
                        continue

                    # Handle product_uom_id mapping untuk bom lines
                    product_uom_id = False
                    if line.get('product_uom_id'):
                        src_uom_id = line['product_uom_id'][0] if isinstance(line['product_uom_id'], list) else line['product_uom_id']
                        src_uom_name = uom_source_id_to_name.get(src_uom_id)
                        if src_uom_name:
                            product_uom_id = uom_name_to_target_id.get(src_uom_name)
                            print(f"🔧 Line UOM Mapping: {src_uom_id} -> {src_uom_name} -> {product_uom_id}")
                    
                    # Jika UOM tidak ditemukan, dapatkan UOM default dari product
                    if not product_uom_id and target_line_product_id:
                        product_uom_id = self.get_default_uom(target_line_product_id)
                        print(f"🔧 Using default UOM for product {target_line_product_id}: {product_uom_id}")
                    ...
                    print(f"✅ Line added: Product {target_line_product_id}, Qty {line['product_qty']}, UOM {product_uom_id}")

                if missing_products:
                    print(f"⚠️ Produk tidak ditemukan di target untuk BoM {record['id']}: {missing_products}")

                # Handle product_uom_id untuk BoM header
                bom_product_uom_id = False
                if record.get('product_uom_id'):
                    src_bom_uom_id = record['product_uom_id'][0] if isinstance(record['product_uom_id'], list) else record['product_uom_id']
                    src_bom_uom_name = uom_source_id_to_name.get(src_bom_uom_id)
                    if src_bom_uom_name:
                        bom_product_uom_id = uom_name_to_target_id.get(src_bom_uom_name)
                        print(f"🔧 Header UOM Mapping: {src_bom_uom_id} -> {src_bom_uom_name} -> {bom_product_uom_id}")

                # Jika tidak ada UOM mapped, get default UOM dari product
                if not bom_product_uom_id and product_id:
                    bom_product_uom_id = self.get_default_uom_from_template(product_id)
                    print(f"🔧 Using default UOM for template {product_id}: {bom_product_uom_id}")

                bom_data = {
                    'product_tmpl_id': product_id,
                    'product_id': product_variant_id or False,
                    'product_qty': record.get('product_qty') or 1.0,
                    'product_uom_id': bom_product_uom_id or False,  # PASTIKAN INI ADA
                ...
                # Update integration status di source
                try:
                    index_store_ids = record.get('index_store', [])
    ...
    def get_default_uom(self, product_id):
        """Get default UOM for a product variant"""
        try:
            product_data = self.target_client.call_odoo('object', 'execute_kw', self.target_client.db,
                self.target_client.uid, self.target_client.password,
                'product.product', 'read',
                [[product_id]], {'fields': ['uom_id']})
            
            if product_data and product_data[0].get('uom_id'):
                return product_data[0]['uom_id'][0]
        except Exception as e:
            print(f"⚠️ Gagal mendapatkan default UOM untuk product {product_id}: {e}")
        
        return False

    def get_default_uom_from_template(self, template_id):
        """Get default UOM for a product template"""
        try:
            template_data = self.target_client.call_odoo('object', 'execute_kw', self.target_client.db,
                self.target_client.uid, self.target_client.password,
                'product.template', 'read',
                [[template_id]], {'fields': ['uom_id']})
            
            if template_data and template_data[0].get('uom_id'):
                return template_data[0]['uom_id'][0]
        except Exception as e:
            print(f"⚠️ Gagal mendapatkan default UOM untuk template {template_id}: {e}")
